...
// Query to get all students organized by house and section
$query = "
    SELECT 
        ...
        RANK() OVER (PARTITION BY h.hid ORDER BY (COALESCE(appreciation_points, 0) + COALESCE(participation_points, 0) + 
                                                  COALESCE(organizer_points, 0) + COALESCE(winner_points, 0) - COALESCE(penalty_points, 0)) DESC) as house_rank,
    ...
    ORDER BY h.name, c.year DESC, c.branch, c.section, c.academic_year DESC, total_points DESC, s.name
";
...
$filename = 'All_Houses_Section_Wise_Summary_' . date('Y-m-d_H-i-s');
...
if ($format === 'csv') {
    ...
    // Write headers
    fputcsv($output, [
        'House', 'Academic Year', 'Year', 'Branch', 'Section', 'Student ID', 'Student Name',
        'Appreciation Points', 'Participation Points', 'Organizer Points', 
        'Winner Points', 'Penalty Points', 'Total Points', 'Performance Grade',
        'House Rank', 'Section Rank'
    ]);
    
    // Write data with house and section grouping
    $current_house = '';
    $current_section = '';
    $house_totals = [];
    $section_totals = [];
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        // Add house header
        if ($row['house_name'] !== $current_house) {
            if ($current_house !== '') {
                // Add house totals
                fputcsv($output, ['', '', '', '', '', '', 'HOUSE TOTAL:', 
                    $house_totals['appreciation'], $house_totals['participation'], 
                    $house_totals['organizer'], $house_totals['winner'], 
                    $house_totals['penalty'], $house_totals['total'], 
                    'Avg: ' . round($house_totals['total'] / $house_totals['count'], 2), '', '']);
                fputcsv($output, ['']); // Empty row
            }
            
            fputcsv($output, ['=== HOUSE: ' . $row['house_name'] . ' ===']);
            $current_house = $row['house_name'];
            $house_totals = ['appreciation' => 0, 'participation' => 0, 'organizer' => 0, 
                           'winner' => 0, 'penalty' => 0, 'total' => 0, 'count' => 0];
        }
        
        // Add year and section headers
        $year_key = 'Year ' . $row['year'];
        $section_key = $row['branch'] . ' - Section ' . $row['section'] . ' (' . $row['academic_year'] . ')';
        $full_section_key = $year_key . ' - ' . $section_key;
        
        // Check if we need a new year header within the house
        if (strpos($current_section, $year_key) !== 0) {
            ...
            // Add year header
            fputcsv($output, ['>> ACADEMIC ' . strtoupper($year_key) . ' <<']);
        }
        ...
        // Update totals
        $house_totals['appreciation'] += $row['appreciation_points'];
        $house_totals['participation'] += $row['participation_points'];
        $house_totals['organizer'] += $row['organizer_points'];
        $house_totals['winner'] += $row['winner_points'];
        $house_totals['penalty'] += $row['penalty_points'];
        $house_totals['total'] += $row['total_points'];
        $house_totals['count']++;
    ...
    // Enhanced Excel Export with house and section grouping
    ...
    // Title
    echo '<h1>All Houses - Section-wise Student Summary</h1>';
    echo '<p>Generated on: ' . date('Y-m-d H:i:s') . '</p>';
    echo '<p>This report shows all students organized by their houses and academic sections with complete point breakdowns.</p>';
    echo '<br>';
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($row['house_name'] !== $current_house) {
            if ($current_house !== '') {
                // Add house totals
                echo '<tr style="background-color: #dc3545; color: white; font-weight: bold; font-size: 14px;">';
                echo '<td colspan="7" style="text-align: right; padding: 10px;">HOUSE TOTAL (' . $current_house . '):</td>';
                echo '<td style="text-align: center;">' . $house_totals['appreciation'] . '</td>';
                echo '<td style="text-align: center;">' . $house_totals['participation'] . '</td>';
                echo '<td style="text-align: center;">' . $house_totals['organizer'] . '</td>';
                echo '<td style="text-align: center;">' . $house_totals['winner'] . '</td>';
                echo '<td style="text-align: center;">' . $house_totals['penalty'] . '</td>';
                echo '<td style="text-align: center; font-size: 16px;">' . $house_totals['total'] . '</td>';
                echo '<td style="text-align: center;">Avg: ' . round($house_totals['total'] / $house_totals['count'], 2) . '</td>';
                echo '<td colspan="2"></td>';
                echo '</tr>';
                echo '<tr><td colspan="16" style="height: 15px; background-color: #ffffff;"></td></tr>'; // Spacer
            }
            ...
            $house_color = $house_colors[$row['house_name']] ?? '#6c757d';
            
            echo '<tr style="background-color: ' . $house_color . '; color: white; font-weight: bold; font-size: 16px;">';
            echo '<td colspan="16" style="text-align: center; padding: 15px;">';
            echo '<i class="fas fa-home"></i> HOUSE: ' . htmlspecialchars($row['house_name']);
            echo '</td></tr>';
        ...
        if (strpos($current_section, $year_key) !== 0) {
            ...
            // Add year header within house
            echo '<tr style="background-color: #fd7e14; color: white; font-weight: bold; font-size: 15px;">';
            echo '<td colspan="16" style="text-align: center; padding: 12px;">';
        ...
        if ($full_section_key !== $current_section) {
            ...
            echo '<td colspan="16" style="text-align: center; padding: 10px;">';
        ...
        if ($row['house_rank'] == 1) $bg_color = '#fff3cd'; // Gold for house topper
        ...
        foreach ($row as $key => $cell) {
            ...
            } elseif ($key === 'house_rank' || $key === 'section_rank') {
