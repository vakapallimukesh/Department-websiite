...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['appreciationStudentId'])) {
    $student_id = $_POST['appreciationStudentId'];
    $event_id = $_POST['appreciationEventId'];
    $points = (int)$_POST['appreciationPoints'];
    ...
    if (empty($student_id) || empty($event_id) || $points < 1 || empty($reason)) {
        ...
        $validate_event_query = "SELECT event_id FROM events WHERE event_id = ?";
        $validate_stmt = mysqli_prepare($conn, $validate_event_query);
        mysqli_stmt_bind_param($validate_stmt, "i", $event_id);
        ...
        if (mysqli_num_rows($validate_result) == 0) {
            $error = "Selected event does not exist. Please choose a valid event.";
        } else {
            // Validate that the student exists
            $validate_student_query = "SELECT student_id FROM students WHERE student_id = ?";
            $validate_student_stmt = mysqli_prepare($conn, $validate_student_query);
            mysqli_stmt_bind_param($validate_student_stmt, "s", $student_id);
            mysqli_stmt_execute($validate_student_stmt);
            $validate_student_result = mysqli_stmt_get_result($validate_student_stmt);
            
            if (mysqli_num_rows($validate_student_result) == 0) {
                $error = "Selected student does not exist. Please choose a valid student.";
            } else {
                $insert_query = "INSERT INTO appreciations (student_id, event_id, points, reason, created_by, created_at) 
                               VALUES (?, ?, ?, ?, ?, NOW())";
                $stmt = mysqli_prepare($conn, $insert_query);
                mysqli_stmt_bind_param($stmt, "siisi", $student_id, $event_id, $points, $reason, $_SESSION['superadmin_username']);
            ...
            mysqli_stmt_close($validate_student_stmt);
        }
        ...
        <div class="row">
            ...
            <nav class="col-md-3 col-lg-2 d-md-block sidebar collapse">
                <div class="position-sticky pt-3">
                    ...
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="#" onclick="showSection('appreciations')">
                                <i class="fas fa-award"></i> Manage Appreciations
                            </a>
                        </li>
                        <li class="nav-item">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 content-area">
                <div class="pt-3 pb-2 mb-3">
                    ...
                    <!-- Appreciations Management Section -->
                    <div id="appreciations-section" class="content-section" style="display: none;">
                        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pb-2 mb-3 border-bottom">
                            <h1 class="h2"><i class="fas fa-award"></i> Manage Appreciations</h1>
                            <div>
                                <button class="btn btn-success btn-custom me-2" onclick="exportAppreciationsData()">
                                    <i class="fas fa-file-excel"></i> Export Data
                                </button>
                                <button class="btn btn-info btn-custom" onclick="loadRecentAppreciations()">
                                    <i class="fas fa-refresh"></i> Refresh
                                </button>
                            </div>
                        </div>
                        
                        <!-- Award Appreciation Points Card -->
                        <div class="card mb-4">
                            <div class="card-header">
                                <h5><i class="fas fa-plus-circle"></i> Award Appreciation Points</h5>
                            </div>
                            <div class="card-body">
                                <form id="awardAppreciationForm">
                                    <div class="row">
                                        <div class="col-md-3 mb-3">
                                            <label for="appreciationStudentId" class="form-label">Student ID:</label>
                                            <input type="text" class="form-control" id="appreciationStudentId" name="appreciationStudentId" placeholder="Enter or search student ID" required>
                                            <div class="form-text">Start typing to search</div>
                                            <div id="appreciationStudentSuggestions" class="dropdown-menu w-100" style="max-height: 200px; overflow-y: auto;"></div>
                                        </div>
                                        <div class="col-md-3 mb-3">
                                            <label for="appreciationEventId" class="form-label">Select Event:</label>
                                            <select class="form-select" id="appreciationEventId" name="appreciationEventId" required>
                                                <option value="">Choose an event...</option>
                                                <!-- Events will be populated here -->
                                            </select>
                                        </div>
                                        <div class="col-md-2 mb-3">
                                            <label for="appreciationPoints" class="form-label">Points:</label>
                                            <input type="number" class="form-control" id="appreciationPoints" name="appreciationPoints" min="1" max="20" required>
                                        </div>
                                        <div class="col-md-3 mb-3">
                                            <label for="appreciationReason" class="form-label">Reason:</label>
                                            <input type="text" class="form-control" id="appreciationReason" name="appreciationReason" placeholder="e.g., Excellent Performance" required>
                                        </div>
                                ...
                                <div id="selectedAppreciationStudentInfo" class="alert alert-info d-none mt-3">
                                    <strong id="selectedAppreciationStudentName"></strong><br>
                                    <small id="selectedAppreciationStudentDetails"></small>
                                </div>
                            </div>
                        </div>
                        
                        <!-- Bulk Award Appreciations -->
                        <div class="card mb-4">
                            <div class="card-header">
                                <h5><i class="fas fa-upload"></i> Bulk Award Appreciations</h5>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-8">
                                        <form id="bulkAppreciationForm" enctype="multipart/form-data">
                                            <div class="row">
                                                <div class="col-md-4 mb-3">
                                                    <label for="appreciationExcelFile" class="form-label">Excel/CSV File:</label>
                                                    <input type="file" class="form-control" id="appreciationExcelFile" name="appreciationExcelFile" accept=".xlsx,.xls,.csv" required>
                                                </div>
                                                <div class="col-md-3 mb-3">
                                                    <label for="bulkAppreciationEventId" class="form-label">Event:</label>
                                                    <select class="form-select" id="bulkAppreciationEventId" name="bulkAppreciationEventId" required>
                                                        <option value="">Choose event...</option>
                                                        <!-- Events will be populated here -->
                                                    </select>
                                                </div>
                                                <div class="col-md-2 mb-3">
                                                    <label for="bulkAppreciationPoints" class="form-label">Default Points:</label>
                                                    <input type="number" class="form-control" id="bulkAppreciationPoints" name="bulkAppreciationPoints" min="1" max="20" value="5" required>
                                                </div>
                                                <div class="col-md-3 mb-3">
                                                    <label for="bulkAppreciationReason" class="form-label">Reason:</label>
                                                    <input type="text" class="form-control" id="bulkAppreciationReason" name="bulkAppreciationReason" placeholder="Default reason" required>
                                                </div>
                                            </div>
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <button type="submit" class="btn btn-primary btn-custom me-2">
                                                        <i class="fas fa-upload"></i> Upload & Award
                                                    </button>
                                                    <a href="download_appreciation_template.php" class="btn btn-success btn-custom">
                                                        <i class="fas fa-download"></i> Download Template
                                                    </a>
                                                </div>
                                            </div>
                                        </form>
                                        <div id="bulkAppreciationResult" class="mt-3"></div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="card">
                                            <div class="card-header">
                                            ...
                                            <div class="card-body">
                                                <small>
                                                    ...
                                                    • Points (uses default if not provided)<br>
                                                    • Reason (uses default if not provided)<br><br>
                                                    
                                                    <strong>Notes:</strong><br>
                                                    • First row should be headers<br>
                                                    • Student IDs must exist in database<br>
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h5><i class="fas fa-history"></i> Recent Appreciations</h5>
                                <div>
                                    <select class="form-select form-select-sm" id="appreciationFilter" onchange="filterAppreciations()">
                            ...
                            <div class="card-body">
                                <div class="row mb-3">
                                    <div class="col-md-4">
                                        <input type="text" class="form-control" id="searchAppreciations" placeholder="Search by student name or ID..." onkeyup="searchAppreciations()">
                                    </div>
                                    <div class="col-md-3">
                                        <select class="form-select" id="appreciationDateFilter" onchange="filterAppreciations()">
                                    ...
                                    <div class="col-md-2">
                                        <select class="form-select" id="appreciationPointsFilter" onchange="filterAppreciations()">
                                            <option value="">All Points</option>
                                            <option value="1-5">1-5 Points</option>
                                            <option value="6-10">6-10 Points</option>
                                            <option value="11+">11+ Points</option>
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="btn-group w-100" role="group">
                                            <button class="btn btn-outline-secondary btn-sm" onclick="resetAppreciationFilters()">
                                                <i class="fas fa-times"></i> Clear
                                            </button>
                                            <button class="btn btn-outline-info btn-sm" onclick="loadRecentAppreciations()">
                                                <i class="fas fa-refresh"></i> Refresh
                                            </button>
                                        </div>
                                    </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        ...
                                        <tbody id="appreciationsTableBody">
                                            <!-- Appreciations will be loaded here -->
                                        </tbody>
                                    </table>
                                </div>
                                
                                <!-- Pagination -->
                                <div id="appreciationsPagination" class="mt-3">
